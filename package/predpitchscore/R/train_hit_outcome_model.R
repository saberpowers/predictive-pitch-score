...
#' @export
#' 
train_hit_outcome_model <- function(pitch, play, base_out_run_exp, tune = FALSE) {
  ...
  data <- pitch |>
    ...
    dplyr::left_join(
      y = play,
      by = c("play_id", "year", "game_id", "event_index", "play_index", "pitch_number")
    ) |>
    ...
    dplyr::transmute(
      ...
      exp_runs_diff = runs_on_play + post_exp_runs - pre_exp_runs
    )
